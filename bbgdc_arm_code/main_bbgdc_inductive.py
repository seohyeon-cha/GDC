...
import time
from model_induct import BBGDCGCN
...
x_train = features[idx_train].cuda()
adj_train = adj[idx_train][:, idx_train].cuda()
adj_train_up = torch.triu(adj_train)
nz_idx_tupl = adj_train_up.nonzero()
nz_idx_train = nz_idx_tupl.T
y_train = labels[idx_train].cuda()
num_nodes_train = len(x_train)
num_edges_train = nz_idx_train.size(1)

x_val = features[idx_val].cuda()
adj_val = adj[idx_val][:, idx_val].cuda()
adj_val_up = torch.triu(adj_val)
nz_idx_tupl = adj_val_up.nonzero()
nz_idx_val = nz_idx_tupl.T
y_val = labels[idx_val].cuda()
num_nodes_val = len(x_val)
num_edges_val = nz_idx_val.size(1)
...
def train_pipeline():
    ...
    for epoch in range(nepochs):
        ...
        output, kld_loss, nll_loss1, nll_loss2, drop_rates = model(x=x_train
                                                            , labels=y_train
                                                            , adj=adj_train
                                                            , nz_idx=nz_idx_train
                                                            , num_edges = num_edges_train
        ...
        if epoch % 100 == 0:
            with torch.no_grad():
                ...
                # acc_test_tr = accuracy(out_mean, y_test)
                ...
                out_runs = torch.stack(outs)
                out_mean = torch.logsumexp(out_runs, dim=0) - np.log(num_run)     
                acc_test_tr = accuracy(out_mean, labels)
...
def multiple_models_CP(model_paths, alpha, num_cal, num_test, mruns):
    ...
    coverage, inefficiency = bayesian_ICP(model_dict, features, idx_train, idx_cal, idx_test, mruns, wup, labels, nz_idx, adj, adj_normt, alpha)
